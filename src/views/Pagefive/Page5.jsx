...
const Page5 = () => {
  ...
  return (
    <div className='h-screen w-full bg-no-repeat bg-cover bg-center 'style={{backgroundImage:`url(${Pagebackgroundimage })`}}>
     <div className='flex flex-col items-centre justify-center text-center  h-120 w-300 m-auto mt-40 '>
        <h2 className='text-4xl/10 text-[rgba(0,0,0,1)] font-bold -mb-6'>Are these challenges slowing <br /> down your <span className='text-[rgba(12,113,224,1)]'> construction business?</span></h2>
      <div className='flex h-65 bg-[rgba(255,255,255,1)] rounded-2xl items-center justify-center text-center p-4 mt-10 '>
        {items?.length >0 ? (
          items?.map((item,index) =>
            <div className='w-full grid grid-cols-4'>
              <Card key={index} id={item?.id} backgroundimages={item?.backgroundimages} image={item?.image} title={item?.title} para={item?.para}   />
            </div>
        )
        ): null}
      </div>
      <button className='flex flex-row h-10 w-35 ml-130 mt-15 items-center justify-center text-center text-[rgba(255,255,255,1)] bg-[rgba(12,113,224,1)] p-3 text-sm rounded-3xl'>Early Access <HiArrowLongRight/> </button>
     </div>
    </div>
  )
}
