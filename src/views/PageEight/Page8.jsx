...
const Page8 = () => {
  ...
  return (
    <div className="h-screen w-full items-center justify-center text-center ">
        ...
        {items?.length > 0
          ? items?.map((item, index) => (
              <div className="h-100 w-120 items-center justify-center text-center flex flex-col ">
                <h1 className="text-3xl font-semibold text-[rgba(12,113,224,1)] relative top-7">
                  {item.title}
                </h1>
                <img
                  className=" h-47 w-85 object-cover m-auto relative top-10 "
                  src={item.image}
                />
                <div
                  className="h-42 w-91 rounded-2xl  "
                  style={{
                    backgroundImage:
                      "linear-gradient(180deg, #0C71E0 0%, #073E7A 100%)",
                  }}
                >
                  <h3 className="text-xl font-semibold text-white mt-20">
      ...
      <button className="flex flex-row items-center justify-center text-center h-10 text-white w-40 p-2 rounded-2xl bg-[rgba(12,113,224,1)] mt-6 ml-150">
        Secure Your Spot <HiArrowLongRight />
      </button>
    </div>
  );
};
